...
python early:
    ...
    def _m1_definitions__mas_periodic_override(self):
        """
    ...
    renpy.audio.audio.Channel.periodic = _m1_definitions__mas_periodic_override
            ...
            for ev in mas_all_ev_db.itervalues():
                if ev.conditional is not None:
                    try:
                        ...
                        raise EventException(
                            "FError al evaluar '{0}' conditional para el evento con '{1}' label:\n{2}.".format(
                                ev.conditional,
                                ev.eventlabel,
    ...
    class MASClickZone(renpy.Displayable):
        ...
        def __init__(self, corners):
            ...
            """
            if len(corners) <= 0:
                raise Exception("La zona clic no puede construirse con esquinas vacías")
...
init 25 python:
    ...
    class PauseDisplayableWithEvents(renpy.Displayable):
        ...
        def __init__(self, events=None, respected_keysims=None):
            ...
            self._m1_definitions__abort_events = False
            self._m1_definitions__should_enable_afm = None
        
        def __repr__(self):
            """
            Representation of this obj
            """
            return "<PauseDisplayableWithEvents ({0})>".format(self._m1_definitions__remaining_events)
        
        @classmethod
        def _m1_definitions__handle_events(cls, events):
            """
            ...
            events.sort(key=cls._m1_definitions__sort_key_td)
        ...
        def set_events(self, events):
            ...
            IN:
                events - PauseDisplayableEvent or a list of PauseDisplayableEvent's
            """
            events = self._m1_definitions__handle_events(events)
            self._m1_definitions__remaining_events = events
            self._m1_definitions__all_events = list(events)
        
        def _m1_definitions__set_end_datetimes(self):
            """
            Sets end datetimes for events using current time
            """
            _now = datetime.datetime.now()
            for ev in self._m1_definitions__remaining_events:
                ev.end_datetime = _now + ev.timedelta
        
        def _m1_definitions__reset_events(self):
            """
            Resets events state
            """
            self._m1_definitions__remaining_events = self._m1_definitions__all_events[:]
            for ev in self._m1_definitions__remaining_events:
        ...
        def start(self):
            ...
            """
            self._m1_definitions__should_enable_afm = store._preferences.afm_enable
            self._m1_definitions__set_end_datetimes()
        ...
        def stop(self):
            ...
            """
            ui.remove(self)
            self._m1_definitions__abort_events = True
            self._m1_definitions__should_enable_afm = None
        ...
        def reset(self):
            ...
            """
            ui.remove(self)
            self._m1_definitions__reset_events()
            self._m1_definitions__abort_events = False
            self._m1_definitions__should_enable_afm = None
        ...
        def _m1_definitions__get_events_for_time(self):
            """
            ...
            for event in self._m1_definitions__remaining_events[:]:
                if _now >= event.end_datetime:
                    self._m1_definitions__remaining_events.remove(event)
                    yield event
        ...
        def _m1_definitions__set_timeout(self):
            """
            Sets a timeout for event generator
            """
            
            if not self._m1_definitions__remaining_events:
                return
            
            _now = datetime.datetime.now()
            _end_dt = self._m1_definitions__remaining_events[0].end_datetime
        ...
        def _m1_definitions__check_keysims(self, ev):
            """
        ...
        def event(self, ev, x, y, st):
            ...
            if ev.type == renpy.display.core.TIMEEVENT:
                for event in self._m1_definitions__get_events_for_time():
                    if not self._m1_definitions__abort_events:
                        event()
                        if event.repeatable:
                            event.set_end_datetime(datetime.datetime.now() + event.timedelta)
                            store.mas_utils.insert_sort(self._m1_definitions__remaining_events, event, PauseDisplayableWithEvents._m1_definitions__sort_key_dt)
                    
                    
                    else:
                        return None
                
                self._m1_definitions__set_timeout()
                raise renpy.IgnoreEvent()
                ...
                self._m1_definitions__abort_events = True
                ui.remove(self)
                if self._m1_definitions__should_enable_afm:
                    self._m1_definitions__should_enable_afm = None
                    store._preferences.afm_enable = True
                return True
            
            
            elif self._m1_definitions__check_keysims(ev):
                raise renpy.IgnoreEvent()
        ...
        @staticmethod
        def _m1_definitions__sort_key_td(ev):
            """
        ...
        @staticmethod
        def _m1_definitions__sort_key_dt(ev):
            """
...
init -995 python in mas_utils:
    ...
    _mas__failrm = "[ERROR] Error al eliminar: '{0}' | {1}"
    _mas__failcp = "[ERROR] Error al copiar: '{0}' -> '{1}' | {2}"
    _mas__faildir = "[ERROR] Error al verificar si es un directorio: {0} | {1}"
